...
第三方 API 和谷歌官方订阅切换与管理工具（Windows 桌面端，WPF + .NET 10，轻量高效，绿色便携）。
...
| 🖥️ **Claude Desktop** | 桌面客户端自定义模型映射与路由接管 | `%APPDATA%\Claude\claude_desktop_config.json` |
...
进入 [Releases](../../releases) 页面，下载最新的 `APISwitch.exe`（单文件绿色版，无需安装任何前置环境，解压即用）。
